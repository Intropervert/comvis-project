...
def main(video_path, offline_deployment, sensor_id):
    ...
    while True:
        ...
        # detections = offline_deployment(numpy_rgb)
        detections = offline_deployment.track(numpy_rgb, persist=True)
                ...
                for box in annot.boxes:
                    ...
                    for track_id in zip(track_ids) : 
                        ...
                        if confidence > confidence_val:
                            ...
                            # Menambahkan data sebagai tuple
                            trackers.append((track_id[0], (left, top, left + width, top + height), class_name))
        ...
        for data in trackers :
            ...
            # print(f"ObjectID: {objectID}, Centroid: {centroid}, BBox: {bbox}, Label: {label}")
            ...
            cv2.putText(frame, str(text) + to.name, (int(centroid[0]) - 10, int(centroid[1]) - 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.rectangle(frame, (int(centroid[0]) - 10, int(centroid[1]) - 10), (int(centroid[0]) + 10, int(centroid[1]) + 10), (0, 255, 0), 3)
            ...
            if demonstrasi_detection: 
                ...
                if start_time_demonstrasi is not None:
                    _current_time_demonstrasi = time.time()
                    _elapsed_time_demonstrasi = _current_time_demonstrasi - start_time_demonstrasi
                    
                    
                    cv2.putText(frame, str(_elapsed_time_demonstrasi), (1, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    ...
                    if result > 0:
                        ...
                        if to.state == 1 :
                            if (time.time() - to.time) > parkir_liar_object_time_limit :
                                ...
                                # KIRIM DATA 
                                # helper.send_mqtt(image_base64, data_result, daqID, camID)
                                cv2.imwrite('media/parkir/' + str(helper.timestamp("%Y%m%d%H%M%S")) + 'parkir.jpg', frame)
            ...
            for i in range(len(objectIDs)):
                for j in range(i + 1, len(objectIDs)):
                    ...
                    if distance > distance_threshold and (time.time() - to.time) > 30:
                        ...
                        # cv2.imwrite('media/objobj/' + str(helper.timestamp("%Y%m%d%H%M%S")) + 'objobj.jpg', frame)
                        print()
        ...
        newf = cv2.resize(frame, (1000, 600))
